<x-app-layout>
                        ...
                        @foreach($users as $user)
                            <a href="{{ route('messages.index', ['user' => $user->id]) }}" 
                               class="flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 group {{ $selectedUserId == $user->id ? 'bg-indigo-600 shadow-xl shadow-indigo-500/20' : 'hover:bg-slate-800 border border-transparent' }}">
                                ...
                                <div class="w-12 h-12 rounded-2xl bg-slate-800 border border-slate-700 flex items-center justify-center font-black text-indigo-400 group-hover:scale-110 transition-transform shadow-inner">
                    ...
                    @if($selectedUserId)
                        ...
                        <div class="p-6 bg-slate-900/50 border-t border-slate-800">
                            <form action="{{ route('messages.store') }}" method="POST" enctype="multipart/form-data" class="flex gap-4 items-center bg-[#1e293b] p-3 rounded-[2rem] border border-slate-700 focus-within:border-indigo-500 focus-within:ring-4 focus-within:ring-indigo-500/10 transition-all">
                                ...
                                <input type="text" name="content" autofocus autocomplete="off" placeholder="Écrire un message..." class="flex-1 bg-transparent border-none text-white focus:ring-0 placeholder-slate-600 text-sm font-medium">
    ...
    <div id="editModal" class="hidden fixed inset-0 bg-[#0f172a]/95 backdrop-blur-md flex items-center justify-center z-50 p-4 transition-all">
        <div class="bg-[#1e293b] border border-slate-700 w-full max-w-md rounded-[3.5rem] p-12 shadow-3xl">
